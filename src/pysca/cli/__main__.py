...
#     pass
    # global _g_ctx
    # if _g_ctx:
    #     _app.ctx.update( _g_ctx.obj['globals'] )
    #     for m in _g_ctx.obj.get('modules',[]):
    #         if hasattr(m,'on_start'):
    #             m.on_start( )

    # if dry==False and query is None:
...
def main(
    ...
    #     mods,syms = core_batch(settings=Path('settings.yaml').absolute())
    #     ctx.obj['modules'] = mods
    #     ctx.obj['globals'].update(syms)
